...
<script type="text/javascript" language="JavaScript">
    ...
    function inclui(){
        ...
        $('[name=idTaxa]').each(function(){
            if ($(this).attr('checked')){
                //alert($(this).val());s
                $('#cobrar').append('<tr><td>'+$(this).parent().html().replace('idTaxa', 'idTaxaCobrar')+'</td></tr>');
    ...
    function exclui(){
        ...
        var qt=0;
        $('[name=idTaxaCobrar]').each(function(){
            if ($(this).attr('checked')){
                qt=1;
                return
            }
        ...
        $('[name=idTaxaCobrar]').each(function(){
            if ($(this).attr('checked')){
                //alert($(this).val());
                $('#naoCobrar').append('<tr><td>'+$(this).parent().html().replace('idTaxaCobrar', 'idTaxa')+'</td></tr>');
                $(this).parent().parent().remove();
            }
        });  
...
<body class="internas">
    ...
    <div id="titulo-subnav">Taxa Administrativa - Inclusão</div>
    <div class="divisoria"></div>
    
    <form action="c" method="POST">
        <input type="hidden" name="app" id="app" value="1221">
              ...
              <td>
                ...
                <div class="recuoPadrao" style="overflow:auto;height:200px;width:200px;">
                    ...
                    <c:forEach var="taxa" items="${taxas}">
                        <c:if test='${taxa.id != 3 && taxa.id != 8 && taxa.tipo!="I" }'>
                            <tr>
                                <td>
                                    <input type="checkbox" name="idTaxa" value="${taxa.id}" >
                                    ${taxa.descricao}<br>
                                </td>
                            </tr>
                        </c:if>                    
