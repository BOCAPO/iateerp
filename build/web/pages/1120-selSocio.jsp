...
<body class="internas">
    ...
    <div id="titulo-subnav">Exame Médico - Seleção de Sócio</div>
    <div class="divisoria"></div>
    
    <form action="c" method="POST" onsubmit="return validarForm()">
        <input type="hidden" name="app" value="1120">
        ...
        <table class="fmt" align="left" >
            <tr>
              ...
              <td>
                  <table class="fmt" style="margin-left:50px;margin-top:25px;">
                      <tr>
                          <td style="background-color: green;color: #000;">&nbsp;&nbsp;Aprovado&nbsp;&nbsp;</td>
                          <td style="background-color: red;color: #000;">&nbsp;&nbsp;Reprovado&nbsp;&nbsp;</td>
                          <td style="background-color: yellow;color: #000;">&nbsp;&nbsp;Vencido&nbsp;&nbsp;</td>
                      </tr>
                  </table>
              </td>
            </tr>
    ...
    <form action="c" method="POST">
        <input type="hidden" name="app" value="1120">
        <input type="hidden" name="acao" value="listaExame">
        ...
        <table id="tabela" style="width:98%;margin-left:15px;">
            ...
            <c:forEach var="socio" items="${socios}">
                <tr>
                    ...
                    <fmt:formatDate var="dtValidade" value="${socio.dtValidadeExame}" pattern="yyyyMMdd"/>

                    <c:choose>
                    <c:when test='${socio.resultadoExame==null}'>
                        <c:set var="cor" value=""/>
                    </c:when>
                    <c:when test='${socio.resultadoExame=="R"}'>
                        <c:set var="cor" value="red"/>
                    </c:when>
                    <c:otherwise>
                        <c:choose>
                        <c:when test='${dtValidade<agora}'>
                            <c:set var="cor" value="yellow"/>
                        </c:when>
                        <c:otherwise>
                    ...
                    <fmt:formatDate var="dtExame"  value="${socio.dtExame}" pattern="dd/MM/yyyy"/>                    
                    <th scope="col" style="background-color: ${cor};">${dtExame}</th>

                    <th scope="col" style="background-color: ${cor};">
                        <c:if test='${socio.resultadoExame=="A"}'>Aprovado</c:if>
                        <c:if test='${socio.resultadoExame=="R"}'>Reprovado</c:if>
                    </th>

                    <fmt:formatDate var="dtValidadeExame" value="${socio.dtValidadeExame}" pattern="dd/MM/yyyy"/>                    
                    <th scope="col" style="background-color: ${cor};">${dtValidadeExame}</th>
                    <th scope="col" style="background-color: ${cor};">${socio.conclusaoExame}</th>
