...
<body class="internas">
    ...
    <script type="text/javascript" language="JavaScript">
        ...
        function matricular(validadeTurma, id){

            if(validadeTurma < <%=new java.util.Date().getTime()%>){
                alert('Para fazer matricula é preciso escolher uma turma ativa');
                return;                
            }else{
                window.location.assign("c?app=3040&acao=showFormSelecionarSocio&idTurma=" + id);
            }
        }
    ...
    <div id="titulo-subnav">Matrícula</div>
    <div class="divisoria"></div>
    ...
    <table id="tabela" style="width:98%;margin-left:15px;"  >
        ...
        <c:forEach var="row" items="${rs.rows}">
            ...
            <c:set var="validadeTurma" value="${row.DT_FIM_TURMA}"/>
            <%
            java.util.Date validadeTurma = (java.util.Date)pageContext.getAttribute("validadeTurma");
            long timeMilis = validadeTurma.getTime();
            %>
            <tr height="1">
                <td class="column1" align="left"><a href="javascript: matricular(<%=timeMilis%>, ${row.SEQ_TURMA})">${row.DESCR_CURSO}</a></td>
                ...
                <c:if test='<%=request.isUserInRole("3070")%>'>
                    <td class="column1" align="center">
                            <a href="c?app=3070&idTurma=${row.SEQ_TURMA}&origem=3040"><img src="imagens/icones/inclusao-titular-04.png" /></a>
