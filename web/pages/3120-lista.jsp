...
<script type="text/javascript" language="JavaScript">
    ...
    function validarForm2(){
        window.location.href='c?app=3121'+
                             '&idCategoria='+document.forms[0].idCategoria.value;
    }
...
<body class="internas">
    ...
    <div id="titulo-subnav">Taxas de Curso</div>
    <div class="divisoria"></div>
    
    <form action="c" method="POST">
        <input type="hidden" name="app" value="3120">
            ...
            <tr>
              <td>
                  ...
                  <div class="selectheightnovo2">
                    <select name="idCategoria" class="campoSemTamanho alturaPadrao larguraComboCategoria" onchange="javascript: document.forms[0].submit()">
                        <c:forEach var="categoria" items="${categorias}">
                            <option value="${categoria.id}" <c:if test="${idCategoria eq categoria.id}">selected</c:if>>${categoria.descricao}</option>
                        </c:forEach>
                    </select>
                  </div>        
              </td>
              ...
              <td> &nbsp;&nbsp;&nbsp;
                <c:if test='<%=request.isUserInRole("3121")%>'>
    ...
    <table id="tabela" style="width:98%;margin-left:15px;">
        ...
        <tbody>
            <c:forEach var="curso" items="${cursos}">
                <tr>
                    <td class="column1" align="left">${curso.deCurso}</td>

                    <fmt:formatDate var="dtInicio" value="${curso.dtInicial}" pattern="dd/MM/yyyy HH:mm:ss" />
                    <td class="column1" align="center">${dtInicio}</td>

                    <fmt:formatNumber var="valor"  value="${curso.valor}" pattern="#0.00"/>
                    <td class="column1" align="center">${valor}</td>

                    <fmt:formatDate var="dtFim" value="${curso.dtFinal}" pattern="dd/MM/yyyy HH:mm:ss" />
                    <td class="column1" align="center">${dtFim}</td>

                    <c:if test='<%=request.isUserInRole("3122")%>'>
                        <th class="column1" align="center">
                            <a href="c?app=3122&acao=showForm&idCategoria=${idCategoria}&idCurso=${curso.curso}&dtInicio=${dtInicio}&dtFim=${dtFim}&valor=${valor}"><img src="imagens/icones/inclusao-titular-03.png"/></a>
                        </th>
                    </c:if>
                </tr>
            </c:forEach>
