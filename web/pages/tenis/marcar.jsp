...
<html>
            ...
            function ajaxUpdate() {
                $.getJSON('ajax/marcar', {
                    "id": "${quadra.id}"
                        ...
                        if (quadra.restricao === 'SIMPLES') {
                            jogo.restricao = 'SIMPLES';
                            $('.botao-simples').attr('disabled', null);
                            $('.botao-duplas').attr('disabled', 'disabled');
                            if (jogo.tipo !== null && jogo.tipo !== 'SIMPLES') {
                                jogo.tipo = null;
                                janelaJogo();
                            }
                        }
                        
                        if (quadra.restricao === 'DUPLAS') {
                            jogo.restricao = 'DUPLAS';
                            $('.botao-simples').attr('disabled', 'disabled');
                            $('.botao-duplas').attr('disabled', null);
                            if (jogo.tipo !== null && jogo.tipo !== 'DUPLAS') {
                                jogo.tipo = null;
                                janelaJogo();
                            }
                        }

                        if (quadra.restricao === undefined || quadra.restricao === '') {
                            jogo.restricao = null;
                            $('.botao-simples').attr('disabled', null);
                            $('.botao-duplas').attr('disabled', null);
                        }
                        ...
                        if (jogo.tipo === 'SIMPLES') {
                            ...
                            s = s + '<p id="court-restriction">Jogo Simples</p>';
                            s = s + '<p id="court-interval">' + quadra.simples + '</p>';
                        }
                        ...
                        if (jogo.tipo === 'DUPLAS') {
                            ...
                            s = s + '<p id="court-restriction">Jogo de Duplas</p>';
                            s = s + '<p id="court-interval">' + quadra.duplas + '</p>';
                        }
                        ...
                        if (jogo.pronto) {
                            s = s + '<div id="schedule-button" class="active-button" onclick="schedule();" onmousedown="$(this).css(\'border-style\', \'inset\');">';
                            s = s + '<p style="text-align: center;">Marcar Jogo</p></div>';
                        } else
                            s = s + '<div id="schedule-button" class="inactive-button"><p style="text-align: center;">Marcar Jogo</p></div>';
                        
                        switch(quadra.estado) {
                            case 'LIVRE':
                                s += '<div id="green-light"></div>';
                                break;
                            case 'OCUPADA':
                                s += (quadra.podeMarcar) ? '<div id="yellow-light"></div>' : '<div id="red-light"></div>';
                                break;
                            case 'BLOQUEADA':
                                s += (quadra.podeMarcar) ? '<div id="yellow-light"></div>' : '<div id="red-light"></div>';
                                break;
                        ...
                        if (!quadra.podeMarcar) {
                            broadcast('Marcação não permitida para a quadra', 'red');
                            $('#player1').css('opacity', '0.3').find('input').attr('disabled', 'disabled');
                            $('#player2').css('opacity', '0.3').find('input').attr('disabled', 'disabled');
                            $('#player3').css('opacity', '0.3').find('input').attr('disabled', 'disabled');
                            $('#player4').css('opacity', '0.3').find('input').attr('disabled', 'disabled');
    ...
    <body>
        ...
        <div id="selecionar-tipo-jogo" style="display: none;">
            <div style="width: 400px; height: 100px; overflow-y: auto; margin: 50px auto 0px auto; text-align: center;">
                <button class="botao-simples" type="button" onclick="selecionaJogo($(this));" disabled="disabled" style="width: 180px; font-family: Arial; font-weight: bold; font-size: 150%;">
                    Simples
                </button>
                <button class="botao-duplas" type="button" onclick="selecionaJogo($(this));" disabled="disabled" style="width: 180px; font-family: Arial; font-weight: bold; font-size: 150%;">
                    Duplas
                </button>
            </div>
        </div>
