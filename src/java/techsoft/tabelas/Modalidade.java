...
public class Modalidade {
    ...
    private static final Logger log = Logger.getLogger("techsoft.tabelas.Modalidade");
    ...
    public static List<Modalidade> listar(){
        ...
        String sql = "SELECT * FROM TB_MODALIDADE_CURSO ORDER BY 2";
        Connection cn = null;
    ...
    public static Modalidade getInstance(int id){
        Modalidade d = null;
        String sql = "SELECT * FROM TB_MODALIDADE_CURSO WHERE CD_MODALIDADE = ?";
        Connection cn = null;
    ...
    public void excluir(Auditoria audit){
        ...
        try {
            String sql = "DELETE FROM TB_MODALIDADE_CURSO WHERE CD_MODALIDADE = ?";
            cn = Pool.getInstance().getConnection();
            PreparedStatement p = cn.prepareStatement(sql);
            p.setInt(1, id);
            p.executeUpdate();
    ...
    public void inserir(Auditoria audit)throws InserirException{
        ...
        try {
            cn = Pool.getInstance().getConnection();
            PreparedStatement p = cn.prepareStatement("SELECT * FROM TB_MODALIDADE_CURSO WHERE DESCR_MODALIDADE = ?");
            p.setString(1, descricao);
            ResultSet rs = p.executeQuery();

            if(rs.next()){
                String err = "Modalidade já cadastrado " + descricao;
                log.warning(err);
                throw new InserirException(err);
            }else{
                String sql = "INSERT INTO TB_MODALIDADE_CURSO (DESCR_MODALIDADE) VALUES (?)";
    ...
    public void alterar(Auditoria audit){
        ...
        try {
            String sql = "UPDATE TB_MODALIDADE_CURSO SET DESCR_MODALIDADE = ? WHERE CD_MODALIDADE = ?";
            cn = Pool.getInstance().getConnection();
            PreparedStatement p = cn.prepareStatement(sql);
            p.setString(1, descricao);
            p.setInt(2, id);
