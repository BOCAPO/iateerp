...
public class EmprestimoSauna{

    private int id;
    private Socio pessoa;
    private Date dtEmprestimo;
    
    private static final Logger log = Logger.getLogger("techsoft.clube.EmprestimoSauna");
    ...
    public static EmprestimoSauna getInstance(int id){
        EmprestimoSauna d = null;
        CallableStatement cal = null;
        Connection cn = null;
        cn = Pool.getInstance().getConnection();

        try {
            cal = cn.prepareCall("{call SP_EMPRESTIMO_SAUNA('N', null, ?, null, null, null)}");
            cal.setInt(1, id);
            ResultSet rs = cal.executeQuery();
            if(rs.next()){
                d = new EmprestimoSauna();
    ...
    public void inserir(Auditoria audit)throws InserirException{
        ...
        try {
            cal = cn.prepareCall("{call SP_EMPRESTIMO_SAUNA ('I', null, null, ?, ?, ?)}");
            ...
            if (rs.next()){
                if (rs.getString("MSG").equals("OK")){
                    ...
                    audit.registrarMudanca("{call SP_EMPRESTIMO_SAUNA ('I', null, null, ?, ?, ?)}", String.valueOf(pessoa.getMatricula()), String.valueOf(pessoa.getIdCategoria()), String.valueOf(pessoa.getSeqDependente()));
                }else{
