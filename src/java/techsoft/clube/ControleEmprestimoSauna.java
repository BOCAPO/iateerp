...
@Controller
public class ControleEmprestimoSauna{
    ...
    throws ServletException, IOException {
            ...
            try{
                ...
                idTipoArmario = TipoArmarioSauna.listar().get(0).getId();
            }
            
            request.setAttribute("idTipoArmario", idTipoArmario);
            request.setAttribute("tipoArmarios", TipoArmarioSauna.listar());
            request.getRequestDispatcher("/pages/2680-lista.jsp").forward(request, response);
    }
    ...
    throws ServletException, IOException {
        ...
        if("showForm".equals(acao)){
            ...
            request.setAttribute("armario", ArmarioSauna.getInstance(armario));
            request.setAttribute("emprestimo", EmprestimoSauna.getInstance(emprestimo));
            ...
            request.getRequestDispatcher("/pages/2680.jsp").forward(request, response);
            ...
            EmprestimoSauna e = new EmprestimoSauna();
            e.setPessoa(Socio.getInstance(matricula, dependente, categoria));

            Auditoria audit = new Auditoria(request.getUserPrincipal().getName(), "2681", request.getRemoteAddr());
            try{
                e.inserir(audit);
                
                ItemEmprestimoSauna i = new ItemEmprestimoSauna();
                i.setEmprestimoSauna(e);
                
                if(armario>0){
                    i.setArmario(ArmarioSauna.getInstance(armario));
                    i.inserir(audit);
                }
                ...
                e = EmprestimoSauna.getInstance(e.getId());
                ImprimeEmprestimo(e, request, response);
                
                response.sendRedirect("c?app=2680");
            ...
            ItemEmprestimoSauna i = new ItemEmprestimoSauna();
            i.setEmprestimoSauna(EmprestimoSauna.getInstance(emprestimo));
            i.setId(item);
            
            Auditoria audit = new Auditoria(request.getUserPrincipal().getName(), "2682", request.getRemoteAddr());
            try{
                i.devolver(audit, tipoDevolucao, valorMulta, cdTxAdm);
                
                if(emprestimo==0){
                    i = ItemEmprestimoSauna.getInstance(item);
                    ImprimeDevolucao(i.getEmprestimoSauna(), request, response);
                }else{
                    ImprimeDevolucao(EmprestimoSauna.getInstance(emprestimo), request, response);
                }
                
                if(emprestimo>0){
                    response.sendRedirect("c?app=2680");
                }else{
                    response.sendRedirect("c?app=2681&acao=showForm");
                }
    ...
    public static void ImprimeEmprestimo(EmprestimoSauna emprestimo, HttpServletRequest request, HttpServletResponse response){
        ...
        linhas.add("-------------------------------------");
        linhas.add(" ");
        linhas.add("** EMPRESTIMO DE MATERIAL SAUNA **");
        linhas.add(" ");

        Logger log = Logger.getLogger("techsoft.tabelas.EmprestimoSauna");
        Connection cn = null;
        ...
        try {
            ...
            String sql = "SELECT " +
                                "(SELECT DESCR_ARMARIO FROM TB_ARMARIO_SAUNA T0 WHERE T0.CD_ARMARIO = T1.CD_ARMARIO) DESCR_ARMARIO, " +
                                "(SELECT DESCR_TIPO_ARMARIO FROM TB_TIPO_ARMARIO_SAUNA TX, TB_ARMARIO_SAUNA TY WHERE TX.CD_TIPO_ARMARIO = TY.CD_TIPO_ARMARIO AND TY.CD_ARMARIO = T1.CD_ARMARIO) DESCR_TIPO_ARMARIO, " +
                                "CD_ITEM, " + 
                                "COUNT(*) QT_TOALHAS " + 
                         " FROM TB_ITEM_EMPRESTIMO_SAUNA T1 " +
            ...
            while (rs.next()){
                ...
                if (rs.getString("CD_ITEM") != null){
                    ...
                    if(qtToalha > 0){
                        linhas.add(" Qt. Kit: " + String.valueOf(qtToalha));
                    }
                }
    ...
    public static void ImprimeDevolucao(EmprestimoSauna emprestimo, HttpServletRequest request, HttpServletResponse response){
        ArrayList<String> linhas = new ArrayList<String>();
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        SimpleDateFormat sdf2 = new SimpleDateFormat("dd/MM/yyyy");
        DecimalFormat df = new DecimalFormat( "#,###,###,##0.00" );
        ...
        linhas.add("-------------------------------------");
        linhas.add(" ");
        linhas.add("** DEVOLUCAO DE MATERIAL SAUNA **");
