...
public class TipoArmarioSauna {

    private int id;
    private String descricao;
    private static final Logger log = Logger.getLogger("techsoft.clube.TipoArmarioSauna");
    ...
    public static List<TipoArmarioSauna> listar(){
        ArrayList<TipoArmarioSauna> l = new ArrayList<TipoArmarioSauna>();
        CallableStatement cal = null;
        Connection cn = null;
        cn = Pool.getInstance().getConnection();
        
        try {
            cal = cn.prepareCall("{call SP_TIPO_ARMARIO_SAUNA (NULL, NULL, 'C')}");
    ...
    public static TipoArmarioSauna getInstance(int id){
        TipoArmarioSauna d = null;
        CallableStatement cal = null;
        Connection cn = null;
        cn = Pool.getInstance().getConnection();

        try {
            cal = cn.prepareCall("{call SP_TIPO_ARMARIO_SAUNA (?, NULL, 'N')}");
            cal.setInt(1, id);
            ResultSet rs = cal.executeQuery();
            if(rs.next()){
                d = new TipoArmarioSauna();
                d.id = rs.getInt(1);
                d.descricao = rs.getString(2);
            }
        } catch (SQLException e) {
    ...
    public void excluir(Auditoria audit) throws ExcluirException {
        ...
        try {
            cal = cn.prepareCall("{call SP_TIPO_ARMARIO_SAUNA (?, NULL, 'E')}");
            cal.setInt(1, id);
            ResultSet rs = cal.executeQuery();
            if (rs.next()){
                if (rs.getString("MSG").equals("OK")){
                    cn.commit();
                    audit.registrarMudanca("{call SP_TIPO_ARMARIO_SAUNA (?, NULL, 'N')}", String.valueOf(id));
                }else{
    ...
    public void inserir(Auditoria audit)throws InserirException{
        ...
        try {
            cal = cn.prepareCall("{call SP_TIPO_ARMARIO_SAUNA (NULL, ?, 'I')}");
            ...
            if (rs.next()){
                if (rs.getString("MSG").equals("OK")){
                    cn.commit();
                    audit.registrarMudanca("{call SP_TIPO_ARMARIO_SAUNA (NULL, ?, 'I')}", descricao);
                }else{
                    String err = rs.getString("MSG");
                    log.warning(err);
    ...
    public void alterar(Auditoria audit) throws AlterarException{
        ...
        try {
            cal = cn.prepareCall("{call SP_TIPO_ARMARIO_SAUNA (?, ?, 'A')}");
            cal.setInt(1, id);
            ...
            if (rs.next()){
                if (rs.getString("MSG").equals("OK")){
                    cn.commit();
                    audit.registrarMudanca("{call SP_TIPO_ARMARIO_SAUNA (?, ?, 'A')}", String.valueOf(id) , descricao);
                }else{
                    String err = rs.getString("MSG");
                    log.warning(err);
